...
- (void)viewDidLoad {
    ...
    select_view = [[UIView alloc] initWithFrame:CGRectMake(0, 0, screenWide, screenHeight * 0.077)];
    UILabel * title1_label = [[UILabel alloc] initWithFrame:CGRectMake(screenWide * 0.03, screenHeight * 0.025, screenWide * 0.2, screenHeight * 0.03)];
    ...
    [self fundButtonWithArray:select_array andHeight:screenHeight * 0.02 Wide:screenWide * 0.136];
    [self.view addSubview:select_view];
    
    
    UILabel * title2_label = [[UILabel alloc] initWithFrame:CGRectMake(screenWide * 0.03, screenHeight * 0.1, screenWide * 0.2, screenHeight * 0.03)];
    ...
    textView = [[UITextView alloc] initWithFrame:CGRectMake(screenWide * 0.25, screenHeight * 0.1, screenWide * 0.7, screenHeight * 0.35)];
    textView.text = @"你可以聊一聊机构服务,位置，设施等";
    textView.layer.borderColor = [RGB(244, 244, 244) CGColor];
    textView.layer.borderWidth = 1;
    [self.view addSubview:textView];
    
    UILabel * title3_label = [[UILabel alloc] initWithFrame:CGRectMake(screenWide * 0.03, screenHeight * 0.495, screenWide * 0.2, screenHeight * 0.03)];
    ...
    [addImage_btn setBackgroundImage:[UIImage imageNamed:@"add_btn"] forState:UIControlStateNormal];
    [self.view addSubview:addImage_btn];
    UILabel * addImage_label = [[UILabel alloc] initWithFrame:CGRectMake(screenWide * 0.25, screenHeight * 0.58, screenWide * 0.7, screenHeight * 0.02)];
    ...
    [commit_btn setTitle:@"发表点评" forState:UIControlStateNormal];
    [commit_btn addTarget:self action:@selector(pressBtnToSubmit) forControlEvents:UIControlEventTouchUpInside];
...
-(void)fundButtonWithArray:(NSArray *)array andHeight:(CGFloat)height Wide:(CGFloat)wide {
    for (int i = 0; i < array.count; i++) {
        ...
        [btn addTarget:self action:@selector(selectValueBtn:) forControlEvents:UIControlEventTouchUpInside];

...
-(instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    ...
    if (self) {
        UILabel * title_label =[[UILabel alloc] initWithFrame:CGRectMake(8, 0,60, 30)];
        title_label.text = @"收费标准";
        title_label.font = [UIFont systemFontOfSize:14];
        title_label.textAlignment = NSTextAlignmentCenter;
        [self addSubview:title_label];
        for (int i = 0; i < 3; i++) {
            UIView * view = [[UIView alloc] initWithFrame:CGRectMake(0, 30+48 *i, screenWide, 24)];
        ...
        for (int i = 0; i < array.count; i++) {
            for (int j = 0; j < [array[i] count]; j++) {
                UILabel * label = [[UILabel alloc] initWithFrame:CGRectMake(8+screenWide/5 *j,30+48 *i , screenWide/5, 24)];
        ...
        for (int i = 0; i < data_array.count; i++) {
            for (int j = 0; j < [data_array[i] count] ; j++) {
                UILabel * label = [[UILabel alloc] initWithFrame:CGRectMake(8+screenWide/5 *(j+1),54+48 *i , screenWide/5, 24)];

...
-(UITextField *)number_field
{
    if (!_number_field)
    {
        UITextField * number_field = [[UITextField alloc]
                                      initWithFrame:CGRectMake(screenWide * 0.0293, screenHeight * 0.0165, screenWide * 0.7274, screenHeight * 0.0585)];
...
-(UITextField *)passWord_field
{
    if (!_passWord_field)
    {
        UITextField * passWord_field = [[UITextField alloc]
                                        initWithFrame:CGRectMake(screenWide * 0.0293, screenHeight * 0.085, screenWide * 0.7274, screenHeight * 0.0585)];
...
-(UIButton *)reSetpass_btn
{
    if (!_reSetpass_btn)
    {
        ...
        [reSetpass_btn setTitle:@"忘记密码？"
                       forState:UIControlStateNormal];
        [reSetpass_btn addTarget:self
                          action:@selector(resetyourPassword)
        ...
        [reSetpass_btn setTitleColor:RGB(194, 194, 194)
                            forState:UIControlStateNormal];
...
-(UITextField *)test_field
{
    if (!_test_field)
    {
        UITextField * test_field = [[UITextField alloc]
                                    initWithFrame:CGRectMake(screenWide * 0.0293, screenHeight * 0.085, screenWide * 0.4274, screenHeight * 0.0585)];
...
-(UIButton *)getTest_btn
{
    if (!_getTest_btn)
    {
        ...
        [getTest_btn setTitle:@"获取动态验证码"
                     forState:UIControlStateNormal];
        getTest_btn.titleLabel.adjustsFontSizeToFitWidth = YES;
        [getTest_btn addTarget:self
                        action:@selector(getTestPassword)
              forControlEvents:UIControlEventTouchUpInside];
        ...
        [getTest_btn setTitleColor:RGB(109, 109, 109)
                          forState:UIControlStateNormal];
...
-(UIButton *)login_btn
{
    if (!_login_btn)
    {
        ...
        [login_btn addTarget:self
                      action:@selector(loginOrloginView)
            forControlEvents:UIControlEventTouchUpInside];
...
-(UIButton *)regis_btn
{
    if (!_regis_btn)
    {
        ...
        [regis_btn setTitle:@"注册"
                   forState:UIControlStateNormal];
        regis_btn.clipsToBounds = YES;
        regis_btn.layer.cornerRadius = 5;
        [regis_btn addTarget:self
                      action:@selector(regisOrRegisVC)
            forControlEvents:UIControlEventTouchUpInside];
        [regis_btn setTitleColor:RGB(230, 11, 24)
                        forState:UIControlStateNormal];
...
-(instancetype)init
{
    ...
    if (self)
    {
        ...
        bg_view = [[UIView alloc]
                   initWithFrame:CGRectMake(screenWide * 0.107, -screenHeight * 0.24, 0, 0)];
...
-(void)regisOrRegisVC
{
    ...
    }else
    {
        ...
        [_regis_btn setTitleColor:[UIColor whiteColor]
                         forState:UIControlStateNormal];
        [_login_btn setTitleColor:RGB(230, 11, 24)
                         forState:UIControlStateNormal];
...
{
    if (IS_IPHONE5 &&
        textField.frame.origin.y == self.passWord_field.frame.origin.y )
    {
        [UIView animateWithDuration:0.3 animations:^
        {

...
-(instancetype)initWithFrame:(CGRect)frame withMark:(UIImage *)mark andTitle :(NSString *)string {
    ...
    if (self) {
        ...
        self.markImageV = [[UIImageView alloc] initWithFrame:CGRectMake(frame.size.width/6 , frame.size.height/2- frame.size.width/20, frame.size.width/10    , frame.size.width/10)];
       
        _markImageV.image = mark;
        [self addSubview:_markImageV];
        
        self.explainTitle = [[UILabel alloc] initWithFrame:CGRectMake(frame.size.width/3 , frame.size.height/2 - frame.size.width/11, frame.size.width/1.83   , frame.size.width/5.5)];
